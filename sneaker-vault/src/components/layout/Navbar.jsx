    ...
    <>
      ...
      >
        ...
        <Link to="/" className="flex items-center gap-2 group cursor-pointer" onClick={() => window.scrollTo(0,0)}>
          ...
          <span className="font-heading text-lg font-black tracking-widest text-vault-cream uppercase group-hover:text-white transition-colors duration-300">
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={() => setCartOpen(true)} className="relative group cursor-pointer">
            ...
            {totalItems > 0 && (
              <span className="absolute -top-1 -right-1 w-3 h-3 bg-vault-purple text-[8px] flex items-center justify-center rounded-full text-white font-bold group-hover:scale-125 transition-transform duration-300">
                {totalItems}
              </span>
            )}
          </button>
          ...
          >
            ...
            {navLinks.map((link, i) => (
              <motion.a
                ...
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                className="font-heading text-4xl font-black uppercase text-vault-cream hover:text-vault-purple hover:translate-x-4 transition-all duration-400 cursor-pointer"
              >
                {link.name}
              </motion.a>
            ))}
