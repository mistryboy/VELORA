...
export default function DropCalendar() {
  ...
  return (
    <section ref={containerRef} className="relative py-40 border-y border-white/5 bg-vault-black overflow-hidden flex flex-col justify-center min-h-[60vh]">
      ...
      <div className="max-w-7xl mx-auto px-8 relative z-20 w-full mb-16">
          <span className="font-body text-[10px] tracking-[0.5em] uppercase text-white/30 block mb-4">
              Release Radar
          </span>
          <h2 className="font-heading text-5xl md:text-7xl font-black text-vault-cream uppercase tracking-tighter leading-none">
      ...
      <motion.div style={{ x }} className="w-full flex flex-col relative z-20 whitespace-nowrap overflow-hidden">
         {drops.map((drop, i) => (
            <motion.div 
               ...
               className={`group flex flex-col md:flex-row md:items-center justify-between border-b border-white/5 py-8 md:py-12 px-8 md:px-[10%] cursor-crosshair relative transition-all duration-700 ${drop.glow}`}
            >
               <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-16 w-1/2">
                   <div className="font-body text-xs md:text-sm font-bold uppercase tracking-[0.4em] text-white/40 group-hover:text-white transition-colors">
                       {drop.date}
                   </div>
                   <h3 className={`font-heading text-4xl md:text-6xl font-black uppercase tracking-tighter text-white/80 transition-colors duration-500 ${drop.color}`}>
                       {drop.name}
                   </h3>
               </div>
               
               <div className="mt-6 md:mt-0 flex items-center justify-between gap-12">
                   ...
                   <span className="font-body text-[10px] md:text-xs font-bold uppercase tracking-[0.3em] text-vault-purple border border-vault-purple/30 px-6 py-2 rounded-full group-hover:bg-vault-purple group-hover:text-white transition-all">
                       {drop.status}
                   </span>
               </div>
            </motion.div>
