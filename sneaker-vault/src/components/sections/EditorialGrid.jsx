...
export default function EditorialGrid() {
  return (
    <section className="py-32 bg-vault-black px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-8">
            ...
            >
                <h2 className="font-heading text-5xl md:text-7xl font-black text-vault-cream uppercase tracking-tighter leading-none mb-4">
            ...
            <motion.p 
               ...
               viewport={{ once: true }}
               transition={{ duration: 0.8 }}
               className="font-body text-xs text-white/40 max-w-xs uppercase tracking-widest leading-relaxed text-left md:text-right"
            >
            ...
            >
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                 <div className="overflow-hidden mb-2">
                     <span className="block font-body text-[10px] text-vault-purple uppercase tracking-[0.3em] translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                         {article.category}
                     </span>
                 </div>
                 <h3 className="font-heading text-2xl md:text-3xl font-black text-vault-cream uppercase tracking-tight transform group-hover:-translate-y-2 transition-transform duration-500">
                    {article.title}
                 </h3>
                 <div className="h-px w-0 bg-vault-purple mt-4 group-hover:w-12 transition-all duration-700 ease-out" />
              </div>
            </motion.div>
          ))}
        </div>
